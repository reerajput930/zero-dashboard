...
export default function CreateTask() {
  ...
  // first is to fetch the data
   useEffect(()=>{
    ...
    async function fetchData(){ 
      ...
      const response  = await fetch(`https://mern-backend-cdsb.onrender.com/api/singletask/${localStorage.getItem("task._id")}`)
      const data = await response.json()
   
      if (data.status === "success") {
        console.log(data)
        setTask({ taskDesc: data.task.taskDesc,tags:data.task.tags});
  ...
  async function submitted(e) {
    e.preventDefault();
    console.log(task)
    const response = await fetch(`https://mern-backend-cdsb.onrender.com/api/singleupdate/${localStorage.getItem("task._id")}`, {
      method: "PUT",
      headers: {
